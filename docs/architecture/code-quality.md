...
> **`bun.lock` is the source of truth; the root `yarn.lock` is a generated mirror.** Regenerate it with `bun install --yarn` whenever dependencies change — CI regenerates and diffs it (`Verify yarn.lock mirrors bun.lock`) and fails on any drift, so it cannot go stale.
>
> It exists for exactly one reason: Snyk cannot parse `bun.lock`, and with no lockfile it understands it fails the PR check outright with _"Missing required yarn.lock"_ rather than scanning anything. The mirror gives it accurate data instead of nothing.
>
> **Every other lockfile is rejected** (`Reject stray lockfiles`) — nested `yarn.lock`, `package-lock.json`, `pnpm-lock.yaml`. `legacy-v1/yarn.lock` survived the move to bun workspaces still pinning `next@14.2.1`, which carries a critical middleware auth bypass (`GHSA-f82v-jwr5-mffw`) and a dozen highs, while the workspace actually resolved `next@15.5.x`. Nothing installed from it — and that is precisely what made it dangerous. A hand-maintained lockfile nobody installs from changes no installed byte, drifts silently and forever, and is read by scanners as the truth. **A phantom lockfile is worse than no lockfile: it gives confident, specific, wrong answers, and the noise trains you to dismiss the scanner.** The distinction that makes the root mirror safe is that it is _derived and verified on every run_, never edited by hand.
